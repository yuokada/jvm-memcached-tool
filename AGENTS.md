...
## Project Overview
This command-line tool interacts with Memcached using Quarkus and Picocli. The main entry point is `io.github.yuokada.memcached.adapter.in.cli.EntryCommand`, which wires top-level options and subcommands for `display`, `generate`, `dump`, `keys`, `stats`, `sizes`, and `flush`. The standard Maven layout applies: sources under `src/main/java`, resources in `src/main/resources`, and tests in `src/test/java`. Releases are handled through the `maven-release-plugin`.
...
*   `sizes`: Display the `stats sizes` histogram (size in bytes vs count).
